// contextFeedback.js — right panel: tags, feedback, actions, lint gate.
// Owns the six-state resolution UI (docket §Actions).
...
export function resetFeedbackState() {
  feedbackText = ''; editNotesText = ''; editMenuOpen = false; approveMenuOpen = false;
}
...
export function renderContextFeedback(store, onResolve, onChange) {
  ...
  const fbArea = el('textarea.feedback', {
    value: feedbackText,
  ...
  en.append(el('input', {
    type: 'text', value: editNotesText, placeholder: 'e.g. fixed wording in para 3',
...
function renderActions(store, fc, onResolve, onChange) {
  ...
  approveSplit.append(el('button.btn.primary', {
    text: 'Approve ▾', style: 'width:100%',
    onclick: () => { approveMenuOpen = !approveMenuOpen; editMenuOpen = false; onChange(); },
  }));
  ...
  if (editMenuOpen && canEdit) {
    editSplit.append(el('div.menu', {}, [
      el('button', {
        text: 'Commit my edit',
        onclick: () => finish(STATES.EDITED_COMMIT, { reviewerEdit: currentEditValue(fc), editNotes: editNotesText, comment: feedbackText }),
      }),
      el('button', {
        text: 'Send edit as feedback',
        onclick: () => finish(STATES.EDITED_FOR_AGENT, { reviewerEdit: currentEditValue(fc), editNotes: editNotesText, comment: feedbackText }),
      }),
    ]));
  }
  editSplit.append(el('button.btn', {
    text: 'Edit ▾', disabled: !canEdit, style: 'width:100%',
